...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sideview_horizontal.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jjamsa <marvin@42.fr>                      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/24 11:34:55 by jjamsa            #+#    #+#             */
/*   Updated: 2022/04/02 15:19:08 by jjamsa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

t_data	*initialize_sideview_horizontal_row_start(t_data *dst, t_data *src)
{
	int	difference;
	int	row;
...
int	sv_hor_line(t_data *temp, char **now, char **next, char ch)
{
	int	c;
	int	difference;
...
void	next_sv_h_x_point(t_data *prog, char **current, char **next)
{
	int	difference;
...
void	handle_sideview_horizontal_row(int i, t_data *prog)
{
	int		flag;
	int		j;
	t_data	temp;
	...
	while (prog->arr[i][j + 1] != 0 && prog->arr[i + 1] != 0)
	{
		...
		sv_hor_line(&temp, &(prog->arr[i][j]), &(prog->arr[i][j + 1]), 'x');
		if (flag == 0 && prog->arr[i + 1][j] != 0)
		{
			temp = initialize_temp(prog);
			sv_hor_line(&temp, &(prog->arr[i][j]), &(prog->arr[i + 1][j]), 'y');
		}
		else
			flag = 1;
		next_sv_h_x_point(prog, &(prog->arr[i][j]), &(prog->arr[i][j + 1]));
		j++;
	}
	if (flag == 0 && prog->arr[i + 1][j] != 0)
	{
...
int	start_sideview_horizontal(t_data *prog)
{
	...
	{
		handle_sideview_horizontal_row(i, prog);
		prog = initialize_sideview_horizontal_row_start(prog, &start);
		i++;
	}
	j = 0;
